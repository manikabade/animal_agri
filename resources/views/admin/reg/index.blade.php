            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h3>Register</h3><br>
                        @can('user_add')
                        <a class="btn btn-outline-primary " href="{{route('admin.reg.create')}}" role="button">Create Register</a>
                        @endcan
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">List of Client Register</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                {{--                                <div class="row">--}}
                                ...
                                <table id="example1" class="mt-2 table table-bordered table-striped">
                                    ...
                                    @forelse($regs as $reg)
                                        <tr>
                                            ...
                                            <td>{{$reg->password}}</td>
                                            <td>
                                                {!! $reg->status ?'<span class="badge badge-success">Active</span>':'<span class="badge badge-danger">InActive</span>' !!}
                                            </td>

                                            <td>
                                                <div class="row">
                                                    <a class="btn btn-success" href="{{route('admin.reg.edit',$reg->id)}}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
{{--                                                    <form action="{{route('reg.destroy',$reg->id)}}" method="POST">--}}
{{--                                                        @csrf--}}
{{--                                                        @method('DELETE')--}}
...
{{--                                                        </button>--}}
{{--                                                    </form>--}}
                                                    <a href="javascript:void(0)" onclick="var c = confirm('{{ "Are you sure?" }}'); if(c){document.getElementById('delete-{{$reg->id}}').submit();}" class="btn btn-sm swalDefaultQuestion btn-danger">
                                                        <i class="fa fa-trash"></i>
                                                        <form id="delete-{{$reg->id}}" action="{{ route('admin.reg.destroy',$reg->id) }}" method="POST"  style="display: none;">
                                                            @method('DELETE')
                                                            @csrf
                                                        </form>
                                                    </a>
...
maile role admin n customer banayo hoianh
agi frontend ma register fail vanira thiyo backend ma gaera raixa tah feri
backend bata kena bayo banda teyo authorize true xa which check user is login or not
but hami le direct create garda teyo user login hudaina so u have to keep it in mind ok
aba tetei data matra login hunxa ho?
hoina
first. direct user create garda teyo storeregrequest use nagara ..manual validation gara ani
jati pani user direct reg garen sabai customer ho banera assign gara like i did in homecontroller
