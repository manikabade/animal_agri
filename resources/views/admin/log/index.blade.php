            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h3>Login</h3><br>
                        @can('user_add')
                        <a class="btn btn-outline-primary " href="{{route('admin.log.create')}}" role="button">Create Login</a>
                        @endcan
                     </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">List of Signin Client</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                {{--                                <div class="row">--}}
                                ...
                                <table id="example1" class="mt-2 table table-bordered table-striped">
                                    ...
                                    @forelse($logs as $log)
                                        <tr>
                                            ...
                                            <td>{{$log->password}}</td>
                                            <td>
                                                {!! $log->status ?'<span class="badge badge-success">Active</span>':'<span class="badge badge-danger">InActive</span>' !!}
                                            </td>

                                            <td>
                                                <div class="row">
                                                    <a class="btn btn-success" href="{{route('admin.log.edit',$log->id)}}">
...
{{--                                                        </button>--}}
{{--                                                    </form>--}}
                                                    <a href="javascript:void(0)" onclick="var c = confirm('{{ "Are you sure?" }}'); if(c){document.getElementById('delete-{{$log->id}}').submit();}" class="btn btn-sm swalDefaultQuestion btn-danger">
                                                        <i class="fa fa-trash"></i>
                                                        <form id="delete-{{$log->id}}" action="{{ route('admin.log.destroy',$log->id) }}" method="POST"  style="display: none;">
                                                            @method('DELETE')
                                                            @csrf
                                                        </form>
                                                    </a>
