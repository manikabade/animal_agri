        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h3>Goat</h3><br>
                        <a class="btn btn-outline-primary " href="{{route('admin.goat.create')}}" role="button">Create Goat</a>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{url('dashboard')}}"><i class="fa fa-home"></i>Home</a>
                            </li>
                            <li class="breadcrumb-item active">Goat</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">List of Goat</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                {{--                                <div class="row">--}}
                                ...
                                <table id="example1" class="mt-2 table table-bordered table-striped">
                                    ...
                                    @forelse($goats as $goat)
                                        <tr>
                                            <td>{{$goat->id}}</td>
                                            <td>{{$goat->item_name}}</td>
                                            <td>{{$goat->rupees}}</td>
                                            <td>{{$goat->quantity}}</td>
                                            <td>{{$goat->description}}</td>
                                            <td>

                                                <img height="100px" width="100px" src="{{asset('images/goat/'.$goat->image)}}">
                                            </td>

                                            <td>
                                                {!! $goat->status ?'<span class="badge badge-success">Active</span>':'<span class="badge badge-danger">InActive</span>' !!}
                                            </td>

                                            <td>
                                                <div class="row">
                                                    <a class="btn btn-success" href="{{route('admin.goat.edit',$goat->id)}}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
{{--                                                    <form action="{{route('goat.destroy',$goat->id)}}" method="POST">--}}
{{--                                                        @csrf--}}
{{--                                                        @method('DELETE')--}}
...
{{--                                                        </button>--}}
{{--                                                    </form>--}}
                                                    <a href="javascript:void(0)" onclick="var c = confirm('{{ "Are you sure?" }}'); if(c){document.getElementById('delete-{{$goat->id}}').submit();}" class="btn btn-sm swalDefaultQuestion btn-danger">
                                                        <i class="fa fa-trash"></i>
                                                        <form id="delete-{{$goat->id}}" action="{{ route('admin.goat.destroy',$goat->id) }}" method="POST"  style="display: none;">
                                                            @method('DELETE')
                                                            @csrf
                                                        </form>
                                                    </a>
